==== v0.X.N ($Id: README_FIRST.txt 17015 2020-04-05 01:56:33Z NiLuJe $) ====
...
(Still, a note to the poor Windows < 8 users: Yes, that means you'll need a non-default driver.)
...
If that's relevant on your specific device, the KUAL button will also warn you about it.).
...
* Now, to actually connect to the device, we'll need to bring up the shiny usb network interface
...
Note to OS X users: You'll have to configure the network interface manually via OS X's GUI (c.f., http://www.mobileread.com/forums/showpost.php?p=2895606&postcount=13).
...
Bundles a number of useful tools (OpenSSH, rsync, mosh, sshfs, kindletool, htop, lsof, FBInk, fbgrab, strace, ltrace, elfutils, nano, zsh, ag, gdb, tmux, among others, check the README for credits).
...
Don't forget to run login shells (usually by passing the -l flag to your shell).

=== BUGS ===

In order to avoid the "nothing is exported over usb ms" issue after an update,
your Kindle will automatically switch back to USB MS before launching the UYK update process.
...
This is not an issue when you're using MRPI.

In the same vein, if you're not actively debugging a boot process issue,
avoid booting your Kindle while plugged to a computer, it tends to make weird things happen on most devices…
...
Staying on the topic of device-specific quirks, on the KOA2, trying to switch from USBNet to USBMS while *NOT* being plugged in
will most likely result into a deadlock or a kernel panic, for some strange and mysterious reason.
To try to avoid the issue, the script will prevent you from doing just that, while KUAL, unlike on every other device, will
allow you to toggle USBNet while being plugged in, since it appears to be the only way to potentially avoid the issue...


There *might* still be some timing issues left around the IP assignation (noticeable if your kindle blocked for 15s when toggling un):
